...
class TestFXSabrSurface:
    ...
    def test_z_eurusd_surface_cookbook(self):
        from rateslib import add_tenor, Solver, IRS, XCS, FXSwap, FXStraddle, FXRiskReversal, FXBrokerFly
        ...
        estr_swaps = [IRS(dt(2024, 5, 30), _, spec="eur_irs", curves="estr") for _ in
                      mkt_data["tenor"]]
        estr_rates = mkt_data["estr"].tolist()
        labels = mkt_data["tenor"].to_list()
        sofr_swaps = [IRS(dt(2024, 5, 30), _, spec="usd_irs", curves="sofr") for _ in
                      mkt_data["tenor"]]
        ...
        fxswaps = [FXSwap(dt(2024, 5, 30), _, pair="eurusd", curves=[None, "eurusd", None, "sofr"])
                   for _ in mkt_data["tenor"][0:14]]
        fxswap_rates = mkt_data["fx_swap"][0:14].tolist()
        xcs = [XCS(dt(2024, 5, 30), _, spec="eurusd_xcs", curves=["estr", "eurusd", "sofr", "sofr"])
        ...
        surface = FXSabrSurface(
            ...
            node_values=[[0.02, 1.0, 0.01, 0.5]] * 23,
            pair="eurusd",
            delivery_lag=2,
            calendar="tgt|fed",
            id="eurusd_vol"
        ...
        for row in range(11):
            instruments_le_1y.extend([
                FXStraddle(strike="atm_delta", expiry=vol_data["expiry"][row], delta_type="spot",
                           **fx_args),
                FXRiskReversal(strike=("-25d", "25d"), expiry=vol_data["expiry"][row],
                               delta_type="spot", **fx_args),
                FXBrokerFly(strike=(("-25d", "25d"), "atm_delta"), expiry=vol_data["expiry"][row],
                            delta_type="spot", **fx_args),
                FXRiskReversal(strike=("-10d", "10d"), expiry=vol_data["expiry"][row],
                               delta_type="spot", **fx_args),
                FXBrokerFly(strike=(("-10d", "10d"), "atm_delta"), expiry=vol_data["expiry"][row],
                            delta_type="spot", **fx_args),
            ])
            rates_le_1y.extend(
                [vol_data["atm"][row], vol_data["25drr"][row], vol_data["25dbf"][row],
                 vol_data["10drr"][row], vol_data["10dbf"][row]])
            labels_le_1y.extend(
        ...
        for row in range(11, 23):
            instruments_gt_1y.extend([
                FXStraddle(strike="atm_delta", expiry=vol_data["expiry"][row], delta_type="forward",
                           **fx_args),
                FXRiskReversal(strike=("-25d", "25d"), expiry=vol_data["expiry"][row],
                               delta_type="forward", **fx_args),
                FXBrokerFly(strike=(("-25d", "25d"), "atm_delta"), expiry=vol_data["expiry"][row],
                            delta_type="forward", **fx_args),
                FXRiskReversal(strike=("-10d", "10d"), expiry=vol_data["expiry"][row],
                               delta_type="forward", **fx_args),
