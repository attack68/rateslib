...
class BondMixin:
    ...
    def oaspread(
        ...
        return self._oaspread_algorithm(curves_[0], _validate_curve_not_no_input(curves_[1]), metric, _dual_float(price))

    def _oaspread_algorithm(self, curve: CurveOption_, disc_curve: Curve, metric: str, price: float) -> float:
        """
        ...
        npv_price = self.rate(curves=[curve_, disc_curve_], metric=metric)
        b: float = gradient(npv_price, ["z_spread"], 1)[0]
        c: float = _dual_float(npv_price) - _dual_float(price)
        z_hat: float = -c / b
...
class IndexFixedRateBond(FixedRateBond):
    ...
    def _period_cashflow(
        self, period: IndexCashflow | IndexFixedPeriod, curve: Curve_
    ) -> DualTypes:
    ...
    def __init__(
        ...
        convention: str_ = NoInput(0),
        fixed_rate: DualTypes_ = NoInput(0),
        index_base: DualTypes_ | Series[DualTypes] = NoInput(0),
        index_fixings: DualTypes_ | Series[DualTypes] = NoInput(0),
    ...
    def index_ratio(self, settlement: datetime, curve: Curve_) -> DualTypes:
        ...
        index_val: DualTypes = IndexMixin._index_value(
        ...
        index_base: DualTypes = IndexMixin._index_value(
...
class FloatRateNote(Sensitivities, BondMixin, BaseMixin):
    ...
    def _period_cashflow(self, period: Cashflow | FloatPeriod, curve: Curve_) -> DualTypes:
    ...
    def _accrual_rate(self, pseudo_period: FloatPeriod, curve: CurveOption, method_param: int):
        """
