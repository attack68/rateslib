...
class FloatPeriod(BasePeriod):
    ...
    **publication date**, which usually lags the reference value dates.
    ...
    def _maybe_get_cal_and_conv_from_curve(self, curve: Curve | dict | NoInput) -> tuple[CalTypes, str]:
    ...
    def _rate_rfr(self, curve: Curve | dict | NoInput) -> Number:
        ...
        elif isinstance(self.fixings, list):
            # try to calculate rate purely from the fixings
            return self._rfr_rate_from_individual_fixings(curve)
    ...
    def _rfr_get_series_with_populated_fixings(self, obs_dates):
        ...
        if not isinstance(self.fixings, NoInput):
            ...
            assert not isinstance(self.fixings, (float, Dual, Dual2, Variable))
