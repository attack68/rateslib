...
def _d_sabr_d_k(
    ...
    dSABR/dk = dA/dk BCD + A dB/dk CD + AB dC/dk D + ABC dD/dk
    ...
    .. code::
       ...
       sym.pycode(sym.diff(A, k))

       z = v/a*(f*k)**((1-b)/2)*sym.log(f/k)
       B = z
       sym.pycode(sym.diff(B, k))

       chi = sym.log(((1-2*p*z+z*z)**(1/2)+z-p)/(1-p))
       C = chi**-1
       sym.pycode(sym.diff(C, k))

       D = 1+((1-b)**2/24*a**2/(f*k)**(1-b)+(1/4)*(p*b*v*a)/(f*k)**((1-b)/2)+(2-3*p*p)*v*v/24)*t
       sym.pycode(sym.diff(D, k))

    """
    A = a / (
        (f * k) ** ((1 - b) / 2)
        * (1 + (1 - b) / 24 * dual_log(f / k) ** 2 + (1 - b) ** 4 / 1920 * dual_log(f / k) ** 4)
    )
    B = v / a * (f * k) ** ((1 - b) / 2) * dual_log(f / k)
    C = (dual_log(((1 - 2 * p * B + B * B) ** (1 / 2) + B - p) / (1 - p))) ** -1
    D = (
        1
        + (
            (1 - b) ** 2 / 24 * a**2 / (f * k) ** (1 - b)
            + (1 / 4) * (p * b * v * a) / (f * k) ** ((1 - b) / 2)
            + (2 - 3 * p * p) * v * v / 24
        )
        * t
    )
    dA = _d_A_d_k(k, f, t, a, b, p, v)
    dB = _d_B_d_k(k, f, t, a, b, p, v)
    dC = _d_C_d_k(k, f, t, a, b, p, v)
    dD = _d_D_d_k(k, f, t, a, b, p, v)

    return dA * B * C * D + A * dB * C * D + A * B * dC * D + A * B * C * dD
...
def _d_C_d_k(
    ...
    return -(
        (
            1
            - 2 * p * v * (f * k) ** (1 / 2 - 1 / 2 * b) * dual_log(f / k) / a
            + v**2 * (f * k) ** (1 - b) * dual_log(f / k) ** 2 / a**2
        )
        ** (-0.5)
        * (
            -1.0
            ...
            * dual_log(f / k)
            / (a * k)
            + 1.0 * p * v * (f * k) ** (1 / 2 - 1 / 2 * b) / (a * k)
            + 0.5 * v**2 * (f * k) ** (1 - b) * (1 - b) * dual_log(f / k) ** 2 / (a**2 * k)
            - 1.0 * v**2 * (f * k) ** (1 - b) * dual_log(f / k) / (a**2 * k)
        )
        + v * (f * k) ** (1 / 2 - 1 / 2 * b) * (1 / 2 - 1 / 2 * b) * dual_log(f / k) / (a * k)
        - v * (f * k) ** (1 / 2 - 1 / 2 * b) / (a * k)
    ) / (
        (
            -p
            ...
            ** 0.5
            + v * (f * k) ** (1 / 2 - 1 / 2 * b) * dual_log(f / k) / a
        )
        * dual_log(
            (
                -p
