...
from rateslib.periods import Cashflow, FixedPeriod, FloatPeriod, NonDeliverableFixedPeriod
...
class NonDeliverableFixedLeg(_FixedLegMixin, BaseLeg):  # type: ignore[misc]
    """
    Create a fixed leg composed of :class:`~rateslib.periods.NonDeliverableFixedPeriod` s.
    ...
    def __init__(
        ...
        if self.initial_exchange or self.final_exchange:
            raise ValueError("`initial_exchange` and `final_exchange` are not allowed on "
                             "non-deliverable legs.")
