...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let vars = IndexSet::from_iter((0..1000).map(|x| format!("v{}", x).to_string()));
    let vars2 = IndexSet::from_iter((0..1000).map(|x| format!("u{}", x).to_string()));
    let a2 = Duals::Dual(Dual { real: 2.0, vars: Rc::new(vars), dual: duals_ } );
    let b2 = Duals::Dual(Dual { real: 3.0, vars: Rc::new(vars2), dual: duals_2} );
    ...
    c.bench_function("duals add same ptr vars", |z| z.iter(|| duals_add_bm(&a2, &a2.clone())));
