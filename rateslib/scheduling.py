    ...
    def __init__(
        ...
        if isinstance(effective, str):
            ...
            if self.eval_mode == "swaps_align":
                ...
                effective_: datetime = add_tenor(self.eval_date, effective, "NONE", NoInput(0))
        ...
        if isinstance(termination, str):
            ...
            termination_: datetime = add_tenor(self.effective, termination, "NONE", NoInput(0))
        else:
    ...
    if _is_divisible_months(ueffective, utermination, frequency_months):
        if stub_side == "FRONT":
            ...
            if ueffective.day > comparison.day:
                _ = _add_months(ueffective, frequency_months * direction, "NONE", NoInput(0), NoInput(0))
            ...
            if utermination.day < comparison.day:
                _ = _add_months(utermination, frequency_months * direction, "NONE", NoInput(0), NoInput(0))
                _ = _get_roll(_.month, _.year, roll)
            else:
                _ = utermination
                _ = _get_roll(_.month, _.year, roll)
        ...
        for month_offset in range(1, 12):
            stub_date = _add_months(stub_side_dt, month_offset * direction, "NONE", NoInput(0), NoInput(0))
...
def _generate_regular_schedule_unadjusted(
    ...
    for i in range(int(n_periods)):
        _ = _add_months(_, defaults.frequency_months[frequency], "NONE", NoInput(0), NoInput(0))
        _ = _get_roll(_.month, _.year, roll)
        yield _
