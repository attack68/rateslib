...
@pytest.mark.parametrize("fx_rates", [
...
def test_positions_value():
    ...
    assert all(result == np.array([0, 80.]))
...
def test_fxrates_set_order():
    ...
    expected = np.array([
        Dual2(1.0, "fx_usdnok", [0.]),
        Dual2(8.0, "fx_usdnok", [1.])
    ])
...
def test_restate():
    ...
    assert fxr2.fx_rates == {"gbpusd": Dual(1.25, "fx_gbpusd", [1.]),
...
def test_fxforwards_set_order(usdusd, eureur, usdeur):
    ...
    expected = np.array([
        Dual2(1.0, "fx_usdeur", [0.]),
        Dual2(2.0, "fx_usdeur", [1.])
    ])
...
def test_fxforwards_set_order_list(usdusd, eureur, usdeur):
    fxf = FXForwards(
        ...
        {"usdusd": usdusd, "eureur": eureur, "usdeur": usdeur,
         "usdgbp": usdeur.copy(), "gbpgbp": eureur.copy()}
    )
    fxf._set_ad_order(order=2)
    expected = np.array([
        Dual2(1.0, "fx_usdeur", [0.]),
...
def test_generate_proxy_curve():
    ...
    assert c3[dt(2022, 10, 1)] == Dual(
        0.9797979797979798, ['fx_usdcad', 'fx_usdeur'], [0, 0]
    )
